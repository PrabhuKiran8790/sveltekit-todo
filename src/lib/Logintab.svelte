<script>
  ...
  import { loggedIn, userLoginRegisterFormModal, token, allTodos } from "../stores";
  import { getToken, getAllTodos } from "../utils";
  ...
  >
    ...
    <Label class="space-y-2">
      ...
      <Input
        ...
        name="email"
        size="md"
        disabled={$loggedIn || loginWaiting}
        on:change={() => {
          if (userNotFound) userNotFound = false;
          if (incorrectDetails) incorrectDetails = false;
        }}
      >
        <svg
          slot="left"
      ...
      <Helper>
        {#if userNotFound}
          <span class="text-red-500">User with this email is not found</span>
    ...
    <Button type="submit" class="w-full1" disabled={$loggedIn || loginWaiting}
      >{#if loginWaiting}
